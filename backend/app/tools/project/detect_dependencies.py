...
def extract_dependencies_categorized(project_path: str) -> Dict[str, List[str]]:
    """Reads project configuration files and returns segregated lists of frontend and backend dependencies."""
    project_path = os.path.abspath(project_path)
    ...
    # Parse backend dependencies (Python, Go, Rust, Java)
    for req_file in config_files["requirements.txt"]:
        try:
            with open(req_file, "r", encoding="utf-8", errors="ignore") as f:
                for line in f:
                    ...
                    match = re.match(r"^([a-zA-Z0-9_\-]+)", line_strip)
                    if match:
                        backend_deps.add(match.group(1).lower())
        except Exception:
            pass
            ...
            with open(go_file, "r", encoding="utf-8", errors="ignore") as f:
                ...
                for line in f:
                    ...
                    if in_require_block:
                        match = re.match(r"^([a-zA-Z0-9_\.\-/]+)", line_strip)
                        if match:
                            backend_deps.add(match.group(1))
                    elif line_strip.startswith("require "):
                        match = re.match(r"^require\s+([a-zA-Z0-9_\.\-/]+)", line_strip)
                        if match:
                            backend_deps.add(match.group(1))
    ...
    for pom_file in config_files["pom.xml"]:
        try:
            with open(pom_file, "r", encoding="utf-8", errors="ignore") as f:
                ...
                matches = re.findall(r"<artifactId>([^<]+)</artifactId>", content)
                for match in matches:
                    backend_deps.add(match.strip())
        except Exception:
            pass

    # Parse frontend dependencies (JavaScript/TypeScript from package.json)
    for pkg_file in config_files["package.json"]:
        try:
            with open(pkg_file, "r", encoding="utf-8", errors="ignore") as f:
                data = json.load(f)
                for key in ["dependencies", "devDependencies"]:
                    if key in data:
                        for pkg in data[key].keys():
                            frontend_deps.add(pkg)
        except Exception:
            pass

    return {
        "frontend": sorted(list(frontend_deps)),
        "backend": sorted(list(backend_deps))
    }
